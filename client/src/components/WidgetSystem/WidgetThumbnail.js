...
const WidgetThumbnail = (props) => {
    ...
    function getOverlay() {
        if (selected) {
            return <img src={Selected} alt="Selected" class="overlayImg"></img>
        }else if(hovered){
    ...
    useEffect(() => {
        if (props.activeWidgets.includes(props.widgetName)) {//this widget is in activeWidgets, that means we're selected
            setSelected(true);
        } else {
            setSelected(false);
        }
    ...
    return (
        <div id={props.widgetName + "Thumbnail"} className="widgetThumbnail" onClick={() => {
            if (!selected) {//add since it's selected now:
